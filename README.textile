...
<del>you'll still be required to remove all <code>log.something</code> calls in your javascript source, otherwise IE is going to be mad at you. </del>
Now stubs are generated to prevent Javascript errors if you leave the <code>log</code> calls, note that if you change the namesspace, you'll have to manually edit the helper to use the right namespace
